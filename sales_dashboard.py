...
.stApp{background:#1e293b;color:#f8fafc;}
...
/* Πιο ανοιχτές κάρτες */
.kpi-card{background:#334155;border:1px solid #475569;border-radius:12px;padding:1.4rem 1.6rem;position:relative;overflow:hidden;transition:border-color .2s;}
...
.kpi-label{font-size:.72rem;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8;margin-bottom:.5rem;}
...
.sec-header{display:flex;align-items:center;gap:.6rem;font-size:.7rem;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:#cbd5e1;border-bottom:1px solid #475569;padding-bottom:.6rem;margin:2rem 0 1rem;}
.sec-header span{color:#10b981;}

.dept-row{display:flex;align-items:center;gap:.8rem;padding:.55rem .2rem;border-bottom:1px solid #1e293b;}
.dept-rank{font-family:'DM Mono';font-size:.7rem;color:#94a3b8;width:1.4rem;text-align:right;flex-shrink:0;}
.dept-name{flex:1;font-size:.82rem;color:#f1f5f9;}
.dept-bar-wrap{width:120px;height:4px;background:#475569;border-radius:2px;flex-shrink:0;}
.dept-bar{height:100%;border-radius:2px;background:#10b981;}
.dept-val{font-family:'DM Mono';font-size:.78rem;color:#cbd5e1;width:6rem;text-align:right;flex-shrink:0;}

[data-baseweb="tab-list"]{gap:.5rem;background:transparent;border-bottom:1px solid #475569;}
[data-baseweb="tab"]{background:transparent!important;border:none!important;color:#cbd5e1!important;font-size:.8rem;font-weight:600;letter-spacing:.06em;text-transform:uppercase;padding:.6rem 1.2rem!important;border-radius:6px 6px 0 0!important;}
[aria-selected="true"][data-baseweb="tab"]{color:#10b981!important;background:#334155!important;border-bottom:2px solid #10b981!important;}
...
button[kind="secondary"]{background:#334155!important;color:#f1f5f9!important;border:1px solid #475569!important;}
button[kind="secondary"]:hover{background:#475569!important;}

[data-testid="stDataFrame"]{border:1px solid #475569;border-radius:10px;overflow:hidden;}
[data-baseweb="select"]>div{background:#334155!important;border-color:#475569!important;color:#f8fafc!important;}
[data-testid="stAlert"]{background:#334155;border-color:#475569;color:#e2e8f0;}
.stSpinner>div{border-top-color:#10b981!important;}
details{background:#334155!important;border:1px solid #475569!important;border-radius:10px!important;color:#f8fafc;}
</style>
...
# ══════════════════════════════════════════════════════════
# SMART EMAIL SYNC (ΔΙΟΡΘΩΘΗΚΕ Η ΛΟΓΙΚΗ ΓΙΑ ΝΑ ΦΕΡΝΕΙ ΠΑΛΙΑ)
# ══════════════════════════════════════════════════════════
...
def sync_emails(df: pd.DataFrame) -> tuple[pd.DataFrame, list]:
    ...
    # Λίστα με ημερομηνίες που ΕΧΟΥΜΕ ΗΔΗ (ώστε να προσπερνάμε μόνο αυτές)
    existing_dates = set(df['date'].values) if not df.empty else set()
    
    # Ψάχνουμε σταθερά τις τελευταίες 60 ημέρες για να βρούμε τυχόν κενά!
    ...
    try:
        with MailBox('imap.gmail.com').login(EMAIL_USER, EMAIL_PASS) as mailbox:
            ...
            for msg in mailbox.fetch(criteria, reverse=True, mark_seen=False):
                ...
                # Αν η ημερομηνία του email (ή μια μέρα πριν) υπάρχει ήδη, το προσπερνάμε για εξοικονόμηση χρόνου
                ...
                for att in msg.attachments:
                    ...
                    # Τελικός έλεγχος με την ημερομηνία μέσα από το PDF
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:.5rem;">
  ...
  <div style="text-align:right;">
    ...
    <div style="font-family:'DM Mono';font-size:.75rem;color:#94a3b8;">{current_gr_time}</div>
...
if df.empty:
    ...
    <div style="text-align:center;padding:4rem 2rem;color:#94a3b8;">
      ...
      <div style="font-size:1rem;font-weight:600;color:#cbd5e1;">Δεν υπάρχουν δεδομένα</div>
...
with tab1:
    ...
    try:
        ...
        if depts:
            ...
            st.markdown(
                f'<div style="background:#334155;border:1px solid #475569;border-radius:10px;padding:.5rem 1rem 0;">{rows}</div>',
                unsafe_allow_html=True
            )
        else:
            st.markdown('<div style="color:#94a3b8;font-size:.82rem;padding:1rem;">Δεν υπάρχουν δεδομένα τμημάτων.</div>', unsafe_allow_html=True)
    except: pass
...
with tab3:
    ...
    def cmp_table(title_a, title_b, s_a, s_b):
        def row(label, a, b, euro=True):
            ...
            if b == 0: pct_h = "<span style='color:#94a3b8'>—</span>"
            else:
                pct = (a-b)/b*100
                clr = "#34d399" if pct >= 0 else "#fb7185"
                pct_h = f'<span style="color:{clr}">{"▲" if pct>=0 else "▼"} {abs(pct):.1f}%</span>'
            return (f'<tr style="border-bottom:1px solid #475569;">'
                    f'<td style="padding:.6rem .4rem;font-size:.82rem;color:#cbd5e1;">{label}</td>'
                    f'<td style="padding:.6rem .8rem;font-family:DM Mono;font-size:.82rem;color:#f8fafc;text-align:right;">{fmt(a)}</td>'
                    f'<td style="padding:.6rem .8rem;font-family:DM Mono;font-size:.82rem;color:#94a3b8;text-align:right;">{fmt(b)}</td>'
                    f'<td style="padding:.6rem .8rem;text-align:right;font-size:.82rem;">{pct_h}</td></tr>')
        th = (f'<tr style="background:#1e293b;">'
              f'<th style="padding:.7rem .4rem;font-size:.68rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#cbd5e1;text-align:left;"></th>'
              ...
              f'<th style="padding:.7rem .8rem;font-size:.68rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#cbd5e1;text-align:right;">{title_b}</th>'
              f'<th style="padding:.7rem .8rem;font-size:.68rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#cbd5e1;text-align:right;">Μεταβολή</th></tr>')
        ...
        return (f'<table style="width:100%;border-collapse:collapse;background:#334155;'
                f'border:1px solid #475569;border-radius:10px;overflow:hidden;">'
